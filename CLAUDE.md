...
**RBAC v2 and Settings complete** — roles are admin-editable data now, not a hardcoded
pair: `role_templates`/`role_template_permissions` hold a name and permission set,
`RoleProvisioner` materializes each into spatie's per-location role rows
(`/admin/roles*`, `/admin/permissions`). Admins can also grant a permission directly to
one user at one location (`users.permissions[]`, spatie's long-dormant
`model_has_permissions` table) without inventing a role for it. Back-office access is
permission-based: any active user holding an admin-tier permission anywhere — role or
direct grant — signs in and sees only the sections (`sections[]` on the login
response) and report locations (`report_location_ids`) they actually hold;
`AdminAccess::holdsAnywhere`/`EnsureBackOffice` also pin the presented token to one
minted by admin login, or a supervisor's own staff-session token would let them
straight into the back office. `discounts.requires_supervisor` is enforced, not just
stored — `403 discount_needs_supervisor` — making a cashier-safe discount real for the
first time. Settings (`settings` table, config-fallback business identity) and two
per-location threshold columns (`variance_approval_threshold_cents`,
...
- **Never read role assignments through spatie's `roles()` relation.** It applies
  ...
  which have no register to stand at in the first place. `permissions()` is the same
  relation, generated by the same package the same way — `model_has_permissions`
  direct-table joins for direct grants too, see `App\Domain\Rbac\PermissionAssignments`.
- **`AdminAccess`/`holdsAnywhere` is the back-office authorization primitive — admin
  `FormRequest`s never call bare `can()` (no team context there).** `can()` reads
  whatever permission team context happens to be set, and `EnsureStaffSession` — the
  only thing that ever sets one — never runs on an admin request. A bare `can()` in an
  admin `FormRequest` doesn't error; it silently checks against no team (or a stale
  one), the same failure shape as the `roles()` gotcha above, one layer up. Every admin
  `FormRequest::authorize()` goes through `AuthorizesBackOffice::allowsBackOffice()`
  instead, which is `is_admin || AdminAccess::holdsAnywhere(...)`.
...
- **Adding a frontend dependency does not reach the `make dev` containers.** `npm
  ...
  install` (or rebuild the image). Production is unaffected: `make build` runs `npm ci`
  inside the image.
