...
PAY=$(curl -sf -X POST $API/orders/$OID/payments -H "$D" -H "$S" -H 'If-Match: 4' -H "Idempotency-Key: $(uuidgen)" -H "$J" -d "{\"payment_method_code\":\"CASH\",\"amount_cents\":$TOTAL_A,\"tendered_cents\":20000}")
...
PAY2=$(curl -sf -X POST $API/orders/$OID2/payments -H "$D" -H "$S" -H 'If-Match: 1' -H "Idempotency-Key: $(uuidgen)" -H "$J" -d "{\"payment_method_code\":\"VISA\",\"amount_cents\":$(echo $R | jq .data.order.total_cents),\"reference\":\"auth 004321\"}")
...
REF=$(curl -sf -X POST $API/refunds -H "$D" -H "$S" -H "Idempotency-Key: $(uuidgen)" -H "$J" -d "{\"original_order_id\":\"$OID\",\"payment_method_code\":\"CASH\",\"reason\":\"faulty\",\"lines\":[{\"original_order_line_id\":\"$L1\",\"qty\":\"1\",\"restock\":true}]}")
...
echo "10. Z: sales=$(echo $Z | jq -c .data.sales_by_method) refunds=$(echo $Z | jq -c .data.refunds_by_method) payout=$(echo $Z | jq .data.movements.payout) orders_closed=$(echo $Z | jq .data.orders_closed) expected=$(echo $Z | jq .data.expected_cash_cents)"
