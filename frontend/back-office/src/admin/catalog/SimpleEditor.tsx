...
// Radix `Select.Item` rejects an empty-string value, so "no selection" needs a
// sentinel on the wire between the trigger and `values` — translated back to '' the
// moment it leaves this component; the diff/PATCH logic downstream never sees it.
...
export function SimpleEditor({
  ...
  const [pendingArchive, setPendingArchive] = useState<{ body: Record<string, unknown>; label: string } | null>(null)
  ...
  const submit = (e: FormEvent) => {
    ...
    // Only Tax Rates carry `is_active` through this generic editor (Categories don't),
    // but the check is entity-agnostic: any `is_active: false` in the diff is an
    // archive. UNARCHIVE (the table action) never goes through here, so it needs no
    // confirm. `initial !== null` matters too: archiving is an edit-time concept —
    // creating a new row with Active unchecked isn't "archiving" anything (there's no
    // prior active row to leave), so it must never pop the confirm (a Cancel there
    // would otherwise silently block create).
    if (initial !== null && body.is_active === false) {
      const label = typeof values.name === 'string' && values.name ? values.name : 'this record'
      setPendingArchive({ body, label })
      return
    }
  ...
  return (
    <Card>
      ...
      <form onSubmit={submit} className="flex flex-col gap-md">
        {fields.map((f) => (
          <FieldRow key={f.key} label={f.label}>
            {f.kind === 'select' ? (
              <Select
                value={(values[f.key] as string) || NONE_OPTION}
                onValueChange={(v) => setValues((vals) => ({ ...vals, [f.key]: v === NONE_OPTION ? '' : v }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
              ...
              <Checkbox
                ...
                onCheckedChange={(checked) => setValues((v) => ({ ...v, [f.key]: Boolean(checked) }))}
              />
            ) : (
              <Input
                type="text"
      ...
      <ConfirmDialog
        ...
        message={`Archive ${pendingArchive?.label ?? ''}? It leaves the register catalog but stays in history.`}
        confirmLabel="Archive"
        destructive
        onConfirm={() => {
          if (!pendingArchive) return
          onSave(pendingArchive.body)
